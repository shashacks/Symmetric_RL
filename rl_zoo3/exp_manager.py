...
from stable_baselines3.common.callbacks import BaseCallback, CheckpointCallback, EvalCallback, ProgressBarCallback, EvalCustomCallback
...
class ExperimentManager:
    ...
    def __init__(
        ...
        if self._is_atari:
            if self.algo == 'sa2c' or self.algo == 'sppo':
                if hyperparams['robust_type'] == 'mae':
                    suffix = f"{hyperparams['advantage_type']}_e{str(hyperparams['ent_coef']).replace('.', '_')}_a{str(hyperparams['robust_alpha']).replace('.', '_')}_b{str(hyperparams['robust_beta']).replace('.', '_')}_d{str(hyperparams['decay_term'])}_adv{str(hyperparams['normalize_advantage'])}"
                elif hyperparams['robust_type'] == 'rce':
                    suffix = f"{hyperparams['advantage_type']}_e{str(hyperparams['ent_coef']).replace('.', '_')}_Z{str(hyperparams['rce_Z'])}_a{str(hyperparams['robust_alpha']).replace('.', '_')}_b{str(hyperparams['robust_beta']).replace('.', '_')}_d{str(hyperparams['decay_term'])}_adv{str(hyperparams['normalize_advantage'])}"
                self.log_path = f"{log_folder}/{self.env_name}/ntype:{hyperparams['noisy_type']}_{str(hyperparams['noisy_value'])}/{self.algo[1:]}/{self.algo}/{hyperparams['robust_type']}/{suffix}/s{args.seed}/"
            elif self.algo == 'a2c' or self.algo == 'ppo':
                suffix = f"{hyperparams['advantage_type']}_e{str(hyperparams['ent_coef']).replace('.', '_')}_adv{str(hyperparams['normalize_advantage'])}"
                self.log_path = f"{log_folder}/{self.env_name}/ntype:{hyperparams['noisy_type']}_{str(hyperparams['noisy_value'])}/{self.algo}/{self.algo}/{suffix}/s{args.seed}/"
        else:
            if self.algo == 'sa2c' or self.algo == 'sppo':
                self.bins = hyperparams['bins']
                hyperparams.pop('bins')
                if hyperparams['robust_type'] == 'mae':
                    suffix = f"{hyperparams['advantage_type']}_a{str(hyperparams['robust_alpha']).replace('.', '_')}_b{str(hyperparams['robust_beta']).replace('.', '_')}_bins:{self.bins}_d{str(hyperparams['decay_term'])}_adv{str(hyperparams['normalize_advantage'])}"
                elif hyperparams['robust_type'] == 'rce':
                    suffix = f"{hyperparams['advantage_type']}_Z{str(hyperparams['rce_Z'])}_a{str(hyperparams['robust_alpha']).replace('.', '_')}_b{str(hyperparams['robust_beta']).replace('.', '_')}_bins:{self.bins}_d{str(hyperparams['decay_term'])}_adv{str(hyperparams['normalize_advantage'])}"
                self.log_path = f"{log_folder}/{self.env_name}/ntype:{hyperparams['noisy_type']}_{str(hyperparams['noisy_value'])}/{self.algo[1:]}/{self.algo}/{hyperparams['robust_type']}/{suffix}/s{args.seed}/"
            elif self.algo == 'a2c' or self.algo == 'ppo':
                suffix = f"{hyperparams['advantage_type']}_adv{str(hyperparams['normalize_advantage'])}"
                self.log_path = f"{log_folder}/{self.env_name}/ntype:{hyperparams['noisy_type']}_{str(hyperparams['noisy_value'])}/{self.algo}/{self.algo}/{suffix}/s{args.seed}/"
    ...
    def create_callbacks(self):
        ...
        if self.eval_freq > 0 and not self.optimize_hyperparameters:
            ...
            eval_callback = EvalCustomCallback(
    ...
    def create_envs(self, n_envs: int, eval_env: bool = False, no_log: bool = False) -> VecEnv:
        ...
        if not self._is_atari and (self.algo == 'sa2c' or self.algo == 'sppo'):
            import discretizing_wrapper
            env = discretizing_wrapper.discretizing_wrapper(env, self.bins)
