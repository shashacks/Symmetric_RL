...
SelfSPPO = TypeVar("SelfSPPO", bound="SPPO")


class SPPO(OnPolicyAlgorithm):
    """
    ...
    def train(self) -> None:
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                old_prob =  th.exp(rollout_data.old_log_prob)
                ...
                is_clamped_flag = th.where((1 - clip_range <= ratio) & (ratio <= 1 + clip_range), 1, 0)
                policy_loss_min_flag = th.where(policy_loss_1 <= policy_loss_2, 1, 0)
                result_flag = th.logical_or(is_clamped_flag.bool(), policy_loss_min_flag.bool()).float()


                robust_loss = 0
                # Robust loss
                if self.robust_type == 'mae':
                    robust_loss = (result_flag * (advantages * other_probs - advantages * input_action_prob) / old_prob).mean()
                elif self.robust_type == 'rce':
                    robust_loss = -(result_flag * advantages * self.rce_Z * other_probs / old_prob).mean()
                else:
