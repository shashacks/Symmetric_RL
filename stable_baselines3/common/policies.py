...
class ActorCriticPolicy(BasePolicy):
    ...
    def get_action_probs(self, obs: th.Tensor, actions: th.Tensor) -> Tuple[th.Tensor, th.Tensor, Optional[th.Tensor]]:
        """
        Returns the sum of probabilities for all actions except the input action (sampled),
        and the probability for the input action.
        """
        # Preprocess the observation if needed
        features = self.extract_features(obs)
        if self.share_features_extractor:
            ...
            latent_pi = self.mlp_extractor.forward_actor(pi_features)
        distribution = self._get_action_dist_from_latent(latent_pi)
        all_probs = distribution.get_all_probs()
        input_action_prob = th.exp(distribution.log_prob(actions))
        other_probs = all_probs - input_action_prob
        return other_probs, input_action_prob
