...
class CategoricalDistribution(Distribution):
    ...
    def get_all_probs(self):
        return th.sum(self.distribution.probs, dim=-1)
...
class MultiCategoricalDistribution(Distribution):
    ...
    def get_all_probs(self):
        sum_prob_of_each_dim = th.stack([dist.probs.sum(dim=-1) for dist in self.distribution], dim=-1)
        all_probs = th.prod(sum_prob_of_each_dim, dim=-1) # always 1.0
        return all_probs
